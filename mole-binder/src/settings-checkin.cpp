...
void Settings::build_ui()
{
  ...
  connect(save_username_button, SIGNAL(clicked()), SLOT(handle_save_username()));
  ...
  connect(checkin_combo, SIGNAL(currentIndexChanged(int)), SLOT(handle_checkin_combo_changed(int)));
  ...
  connect (random_cookie_button, SIGNAL(clicked()), SLOT(handle_random_cookie()));
...
void Settings::handle_clicked_checkin_help()
{
  ...
  QMessageBox::information(this, tr("About Check-In"), help);
}
